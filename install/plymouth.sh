...
if ! command -v plymouth &>/dev/null; then
  ...
  if [ -d "/boot/loader/entries" ]; then
    ...
    if ! grep -q splash /etc/cmdline.d/*.conf; then
        # Need splash, create the arch_init file
        echo "splash" | sudo tee -a /etc/cmdline.d/arch_init.conf
    fi
    if ! grep -q quiet /etc/cmdline.d/*.conf; then
        # Need quiet, create or append the arch_init file
        echo "quiet" | sudo tee -a /etc/cmdline.d/arch_init.conf
    fi
  ...
  sudo cp -r "$HOME/.local/share/arch_init/default/plymouth" /usr/share/plymouth/themes/arch_init/

  sudo plymouth-set-default-theme -R arch_init
fi
